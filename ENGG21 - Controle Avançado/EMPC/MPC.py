...
class MPC:
    ...
    def run(self, plant, iter, y_spList):
        ...
        for k in range(iter):
            ...
            ## Passo na planta ** É preciso alterar isso de acordo com seu sistema
            ...
            ## Matrizes de visualização ** É preciso alterar isso de acordo com seu sistema
    ...
    def kalmanFilter(self, iter = 100):
        covMedido = .5 # O quanto você confia na sua medição da planta
        covSistema = .5 # O quanto você confia no modelo do MPC
        sM = self.Atil.shape
        PP = np.eye(sM[1])
    ...
    def plot(self):
        ...
        plt.xlabel('Tempo / s')
        plt.savefig(os.path.join(output_dir, 'MPC_y0_temperaturaCompressor.png'))
        ...
        plt.xlabel('Tempo / s')
        plt.savefig(os.path.join(output_dir, 'MPC_y1_vazaoMassica.png'))

        plt.figure(figsize=(10, 6))
        plt.plot(t,P, label='Pressão no final do duto / kPa')
        plt.plot(t,y_spListDenorm[2], label = 'SetPoint')
        plt.ylabel('Pressão no final do duto / MPa')
        plt.xlabel('Tempo / s')
        plt.savefig(os.path.join(output_dir, 'MPC_y2_pressaoDuto.png'))

        plt.figure(figsize=(10, 6))
        plt.plot(t, W, label='Potência do Compressor / W')
        plt.plot(t,y_spListDenorm[3], label = 'SetPoint')
        plt.ylabel('Potência do Compressor / W')
        plt.xlabel('Tempo / s')
        plt.savefig(os.path.join(output_dir, 'MPC_y3_potencia.png'))

        plt.figure(figsize=(10, 6))
        plt.plot(t, rot, label='Vel. Rotação do Compressor / hz')
        plt.ylabel('Vel. Rotação do Compressor / hz')
        plt.xlabel('Tempo / s')
        plt.savefig(os.path.join(output_dir, 'MPC_u0_rotação.png'))
        
        plt.figure(figsize=(10, 6))
        plt.plot(t, P1, label='Pressão de entrada no compressor / MPa')
        plt.ylabel('Pressão de entrada / MPa')
        plt.xlabel('Tempo / s')
        plt.savefig(os.path.join(output_dir, 'MPC_u1_pressaoEntrada.png'))

        plt.figure(figsize=(10, 6))
        plt.plot(t, T1, label='Temperatura de entrada no compressor / K')
        plt.ylabel('Temperatura de entrada no compressor / K')
        plt.xlabel('Tempo / s')
        plt.savefig(os.path.join(output_dir, 'MPC_u2_temperaturaEntrada.png'))

        plt.figure(figsize=(10, 6))
        plt.plot(t, Vv, label=r'Vazão volumétrica / \frac{kg}{m^3 s^2}')
        plt.ylabel(r'Vazão volumétrica / \frac{kg}{m^3 s^2}')
        plt.xlabel('Tempo / s')
        plt.savefig(os.path.join(output_dir, 'MPC_u3_vazaoVolumetrica.png'))
...
if __name__ == "__main__":
    ...
    Q = np.diag([1,5,10,0]) # Peso das Saídas - shape nY x nY
    R = np.eye(model.nU) # Peso das Entradas - shape nU x nU
    ...
    for i in range(iter):
        if i >= 2:
            y_spList[-3,i] = y_spList[-3,i] - 0.1
        if i >= 7:
            y_spList[-2,i] = y_spList[-2,i] + 0.1
    
    MPC = MPC(model, p, m, Q, R)
    MPC.run(model.plant,iter,y_spList)
    MPC.plot()
